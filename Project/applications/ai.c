...
void AI_en(void *parameter)
{
    ...
    if (rt_ai_init(person_d, work_buf) != 0)
    {
    ...
    while (1)
    {
        ...
        if (ai_flag > 0)
        {
            ...
            {
//                rt_kprintf("detected! \n");
                _x1 = (int)(best_box.x * 320 - (best_box.w * 320 * 0.5));
                _x1 = _x1 > 0 ? _x1 : 1;
                _x1 = _x1 < 320 ? _x1 : 319;
                _y1 = (int)(best_box.y * 240 - (best_box.h * 160 * 1.5 * 0.5));
                _y1 = _y1 > 0 ? _y1 : 1;
                _y1 = _y1 < 240 ? _y1 : 239;
                _x2 = (int)(best_box.x * 320 + (best_box.w * 320 * 0.5));
                _x2 = _x2 > 0 ? _x2 : 1;
                _x2 = _x2 < 320 ? _x2 : 319;
                _y2 = (int)(best_box.y * 240 + (best_box.h * 160 * 1.5 * 0.5));
                _y2 = _y2 > 0 ? _y2 : 1;
                _y2 = _y2 < 240 ? _y2 : 239;
...
{
    rt_thread_t ai_th = rt_thread_create("AI", AI_en, NULL, 1024 * 4, 20, 5);
    if (ai_th != RT_NULL)
        rt_thread_startup(ai_th);

    return RT_EOK;
}
MSH_CMD_EXPORT(ai_init, Start the AI camera to recognize person);
